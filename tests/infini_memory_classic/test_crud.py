...
def test_list_empty(tmp_path):
    ...
    assert m.list(store=STORE, user_id="no_such_user") == []
...
def test_list_after_seed(tmp_path):
    ...
    docs = m.list(store=STORE, user_id="alice")
    assert len(docs) == 1
...
def test_get_existing(tmp_path):
    ...
    doc = m.get(doc_id, store=STORE, user_id="bob")
...
def test_get_nonexistent(tmp_path):
    ...
    assert m.get("nonexistent-id", store=STORE, user_id="bob") is None
...
def test_get_wrong_user(tmp_path):
    ...
    assert m.get(doc_id, store=STORE, user_id="bob") is None
...
def test_update(tmp_path):
    ...
    updated = m.update(doc_id, "new content", "new summary", store=STORE, user_id="alice")
    assert updated["id"] == doc_id
    assert updated["content"] == "new content"
    assert updated["summary"] == "new summary"
    assert updated["update_count"] == 1

    fetched = m.get(doc_id, store=STORE, user_id="alice")
...
def test_update_nonexistent(tmp_path):
    ...
    try:
        m.update("nonexistent-id", "content", "summary", store=STORE, user_id="alice")
...
def test_delete(tmp_path):
    ...
    assert len(m.list(store=STORE, user_id="alice")) == 1
    m.delete(doc_id, store=STORE, user_id="alice")
    assert len(m.list(store=STORE, user_id="alice")) == 0
    assert m.get(doc_id, store=STORE, user_id="alice") is None
...
def test_delete_nonexistent(tmp_path):
    ...
    try:
        m.delete("nonexistent-id", store=STORE, user_id="alice")
        assert False, "Should have raised ValueError"
    except ValueError:
        pass
...
def test_list_users(tmp_path):
    ...
    assert m.list_users(store=STORE) == []

    _seed_doc(data_root, "alice", "content", "summary")
    _seed_doc(data_root, "bob", "content", "summary")

    users = m.list_users(store=STORE)
...
def test_delete_user(tmp_path):
    ...
    _seed_doc(data_root, "alice", "content", "summary")
    _seed_doc(data_root, "bob", "content", "summary")
    assert "alice" in m.list_users(store=STORE)

    m.delete_user("alice", store=STORE)
    assert m.list_users(store=STORE) == ["bob"]
    assert m.list(store=STORE, user_id="alice") == []


def test_delete_user_nonexistent(tmp_path):
    data_root = tmp_path / "data"
    m = _make_memory(data_root)
    m.delete_user("nonexistent", store=STORE)
...
def test_stats(tmp_path):
    ...
    s = m.stats(store=STORE, user_id="alice")
...
def test_history_empty(tmp_path):
    data_root = tmp_path / "data"
    m = _make_memory(data_root)
    assert m.history(store=STORE, user_id="no_such_user") == []
...
def test_history_after_operations(tmp_path):
    ...
    events = m.history(store=STORE, user_id="alice")
    assert len(events) == 1
...
def test_delete_all(tmp_path):
    ...
    assert m.count(store=STORE, user_id="alice") == 3

    deleted = m.delete_all(store=STORE, user_id="alice")
    assert deleted == 3
    assert m.count(store=STORE, user_id="alice") == 0
    assert m.list(store=STORE, user_id="alice") == []
...
def test_delete_all_empty(tmp_path):
    data_root = tmp_path / "data"
    m = _make_memory(data_root)
    assert m.delete_all(store=STORE, user_id="no_such_user") == 0
...
def test_reset(tmp_path):
    ...
    assert len(m.list_users(store=STORE)) == 3
...
def test_search_limit_parameter(tmp_path):
    ...
    result = m.search("test", store=STORE, user_id="no_user", limit=5)
    # Verify cfg is restored after search with limit
    assert m._cfg.memory.search_limit == orig_limit

    result = m.search("test", store=STORE, user_id="no_user")
    assert m._cfg.memory.search_limit == orig_limit
