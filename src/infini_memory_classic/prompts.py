...
EXTRACTION_HINTS_CONTEXT = """**Domain-specific extraction guidance from active skills:**

The user has active skills that indicate areas of particular interest. When extracting information,
pay extra attention to facts related to these domains (in addition to normal extraction):

{hints}

This guidance supplements the standard extraction rules below — do NOT skip general information
just because it doesn't match these hints. The hints only add priority, they do not restrict scope."""

GENERATE_SKILLS_PROMPT = """You are a skill discovery engine for a personal memory system.

Analyze the user's accumulated memory documents and identify recurring patterns, experiences,
workflows, or domain expertise that would benefit from being crystallized into reusable skills.

**Input:**
- Document summaries: {doc_summaries}
...
{{
  "skills": [
    {{
      ...
      "action": "create",
      "description": "One-line description of what this skill covers and when to use it (max 200 chars)",
      "extraction_hints": ["hint1: what to pay attention to during extraction", "hint2"],
      "source_doc_ids": ["doc-id-1", "doc-id-2"],
      "reasoning": "Brief explanation of why this skill is warranted"
    }}
  ]
}}
```
...
**Rules:**
1. Only propose a skill when there are at least 3 related facts across 2+ different documents
2. Maximum {max_skills} skills per invocation
3. Skill names: lowercase, hyphens only, 2-64 characters (e.g., "travel-preferences", "work-habits")
4. Do NOT duplicate or overlap with existing skills
5. extraction_hints: 1-5 short phrases telling the extraction system what domain-specific information to prioritize (e.g., "deadlines and milestone dates", "dietary restrictions and food preferences")
6. source_doc_ids must be actual IDs from the document list
7. Focus on actionable, recurring patterns — not one-time events

**Skill categories to look for:**
- Preferences and habits (dietary, work style, communication style)
- Professional domain knowledge (tools, workflows, conventions)
- Recurring processes (how the user handles certain tasks)
- Important relationships and contexts (team structure, project roles)
- Health, lifestyle, and personal patterns

Output ONLY the JSON object, no explanation."""

WRITE_SKILL_PROMPT = """You are a skill writer for a personal memory system.

Generate a complete SKILL.md file based on the planned skill and source documents.
...
**Source documents (containing the evidence for this skill):**

{source_content}

**Output a complete SKILL.md file** with this format:

```markdown
---
name: {skill_name}
description: <refined description, max 200 chars>
extraction_hints:
  - <hint 1>
  - <hint 2>
---
...
## Key Patterns
<Synthesized patterns, preferences, or knowledge from the source documents>

## Guidelines
<Actionable guidelines or procedures derived from the evidence>
```

**Rules:**
1. Synthesize actionable knowledge — do NOT just list or echo raw facts
2. Keep the body under 2000 tokens
3. The description should be self-contained and useful in a skill listing
4. extraction_hints should be specific enough to guide information extraction
5. Write in the same language as the source documents
6. Include the YAML frontmatter with name, description, and extraction_hints

Output ONLY the SKILL.md content (including the YAML frontmatter), no explanation."""
