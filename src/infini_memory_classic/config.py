...
class MemoryConfig:
    ...
    skills_enabled: bool = True           # Enable skills features
    skills_dir: str = "skills"            # Skills subdirectory name
    skills_auto_generate: bool = False    # Auto-generate skills after splits
    skills_generate_frequency: int = 5    # Generate every N splits (when auto_generate=True)
    skills_max_per_user: int = 20         # Max skills per user
    skills_extraction_hints: bool = True  # Use skills to guide extraction
...
class InfiniMemoryConfig:
    ...
    def __init__(self, root: Optional[Path] = None, config_file: Optional[Path] = None):
        ...
        storage_dict = (raw.get("storage") or {}) if isinstance(raw, dict) else {}
        ...
        self.memory = MemoryConfig(
            ...
            skills_enabled=bool(memory_dict["skills_enabled"]) if "skills_enabled" in memory_dict else MemoryConfig.skills_enabled,
            skills_dir=str(memory_dict["skills_dir"]) if "skills_dir" in memory_dict else MemoryConfig.skills_dir,
            skills_auto_generate=bool(memory_dict["skills_auto_generate"]) if "skills_auto_generate" in memory_dict else MemoryConfig.skills_auto_generate,
            skills_generate_frequency=int(memory_dict["skills_generate_frequency"]) if "skills_generate_frequency" in memory_dict else MemoryConfig.skills_generate_frequency,
            skills_max_per_user=int(memory_dict["skills_max_per_user"]) if "skills_max_per_user" in memory_dict else MemoryConfig.skills_max_per_user,
            skills_extraction_hints=bool(memory_dict["skills_extraction_hints"]) if "skills_extraction_hints" in memory_dict else MemoryConfig.skills_extraction_hints,
        )
        self.storage = StorageConfig(
            storage_type=str(storage_dict["storage_type"]) if "storage_type" in storage_dict else StorageConfig.storage_type,
            s3_endpoint=str(storage_dict["s3_endpoint"]) if "s3_endpoint" in storage_dict else StorageConfig.s3_endpoint,
            s3_bucket=str(storage_dict["s3_bucket"]) if "s3_bucket" in storage_dict else StorageConfig.s3_bucket,
            s3_access_key=str(storage_dict["s3_access_key"]) if "s3_access_key" in storage_dict else StorageConfig.s3_access_key,
            s3_secret_key=str(storage_dict["s3_secret_key"]) if "s3_secret_key" in storage_dict else StorageConfig.s3_secret_key,
            s3_prefix=str(storage_dict["s3_prefix"]) if "s3_prefix" in storage_dict else StorageConfig.s3_prefix,
        )
    ...
    def get_storage_backend(self):
        """Lazily create and cache the storage backend."""
    ...
    def from_kwargs(
        ...
        **memory_kwargs: Any,
        ...
        Args:
            ...
            s3_bucket: S3 bucket name (default "infini-memory").
